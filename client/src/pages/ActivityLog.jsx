...
export default function ActivityLog({ user }) {
  ...
  const actionLabels = {
    sold: t('actions.sold'),
    returned: t('actions.returned'),
    stock_in: t('actions.stockIn'),
    stock_out: t('actions.stockOut'),
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-500 mt-1">{isGm ? t('subtitleGm') : t('subtitleStaff')}</p>
      </div>
          ...
          <table className="w-full">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-sm font-semibold text-gray-900">{t('colAction')}</th>
                {isGm && <th className="px-6 py-3 text-left text-sm font-semibold text-gray-900">{t('colSoldBy')}</th>}
              </tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan="5" className="px-6 py-8 text-center text-gray-500">{t('noActivity')}</td></tr>
              ) : (
                logs.map((log) => (
                  <tr key={log._id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="px-6 py-3 text-sm text-gray-600">{Math.round(log.quantity)}</td>
                    <td className="px-6 py-3">
                      <span className={log.action === 'sold' ? 'badge-green' : 'badge-yellow'}>{actionLabels[log.action] || log.action}</span>
                    </td>
